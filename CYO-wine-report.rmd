...
This is the final report on my "Chose Your Own!" data science project. Based on the briefing, a dataset named "[Wine](https://archive.ics.uci.edu/ml/datasets/Wine)" was found and chosen from the [UCI Machine Learning Repository](https://archive.ics.uci.edu/ml/index.php).  
...
wines %>% subset(select=-Winery) %>% cor(.) %>% corrplot(tl.col="black", tl.cex=0.7, number.cex=0.6, title="Correlation Plot", type="lower", addCoef.col="black",mar=c(0,0,1,0))
```
...
The knn model is already very good - an accuracy of `r cm_knn$overall["Accuracy"]` is achieved. This is quite a good start and might be difficult to improve.
...
The resulting classification tree is surprisingly simple and based on only two predictors - Proline and the OD280/OD315 on diluted wines. However, the accuracy of `r cm_rpart$overall["Accuracy"]` is much worse than the results from the knn approach.  
...
```
The approach using Rborist provided an accuracy of `r cm_rb$overall["Accuracy"]`, the best so far.
...
We can now compare the predictions of the three best models (knn, Rborist and svmLinear) and base our prediction on the rounded average of all three to see if there is an improvement.
